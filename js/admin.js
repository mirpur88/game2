...
class AdminManager {
    ...
    setupRealtimeSubscriptions() {
        console.log('Setting up realtime subscriptions for live updates...');

        // Subscribe to transactions table - updates dashboard stats and transactions list
        this.transactionsChannel = supabase
            ...
            .on('postgres_changes', {
                ...
                // Refresh transactions table if viewing it
        ...
        // Subscribe to profiles table - updates user count and users list
        this.profilesChannel = supabase
            ...
            .on('postgres_changes', {
                ...
                // Refresh dashboard stats for new users
                if (this.currentTab === 'dashboardTab') {
                    this.loadDashboardStats();
                }

                // Refresh users/members lists
        ...
        // Subscribe to payments - most critical for real-time updates
        this.paymentsChannel = supabase
            ...
            .on('postgres_changes', {
                ...
                schema: 'public',
                table: 'transactions',
                filter: 'status=eq.pending' // Only pending payments
            }, (payload) => {
                console.log('Payment request detected:', payload);
    ...
    initPage() {
        ...
        // NEW: Real-time payments listener
    ...
    setupAdminRealtime() {
        console.log('Setting up admin real-time listener for transactions...');
        supabase
            ...
            .on('postgres_changes', {
                ...
                console.log('Real-time transaction event:', payload.eventType, payload.new || payload.old);

                // For any change in transactions, we refresh relevant sections
                // This handles new requests (INSERT), approvals/rejections (UPDATE), and removals (DELETE)

                // 1. Refresh payments requested list
                this.loadPayments();

                // 2. Refresh transactions list if that tab is open
                const txTab = document.getElementById('transactionsTab');
                if (txTab && txTab.classList.contains('active')) {
                    this.loadTransactions();
                }

                // 3. Refresh dashboard stats to show updated counts/amounts
                this.loadDashboardStats();
            })
            .subscribe((status) => {
    ...
    initAdvancedFilters() {
        ...
        ['userStartDate', 'userEndDate', 'userVipFilter', 'userMinBalance', 'userMaxBalance', 'userSort'].forEach(id => {
        ...
        ['txStartDate', 'txEndDate', 'txMinAmount', 'txMaxAmount', 'txTypeFilter'].forEach(id => {
            const el = document.getElementById(id);
        ...
        ['payStartDate', 'payEndDate'].forEach(id => {
            const el = document.getElementById(id);
        ...
        ['memberStartDate', 'memberEndDate', 'memberVipFilter', 'memberMinBalance', 'memberMaxBalance', 'memberSort'].forEach(id => {
            const el = document.getElementById(id);
    ...
    async loadUsers(searchTerm = null) {
        try {
            ...
            const minBal = parseFloat(document.getElementById('userMinBalance')?.value);
            const maxBal = parseFloat(document.getElementById('userMaxBalance')?.value);
            ...
            if (!isNaN(minBal)) query = query.gte('balance', minBal);
            if (!isNaN(maxBal)) query = query.lte('balance', maxBal);
    ...
    async loadTransactions(search = null) {
        try {
            ...
            const minAmt = parseFloat(document.getElementById('txMinAmount')?.value);
            const maxAmt = parseFloat(document.getElementById('txMaxAmount')?.value);
            ...
            if (start) query = query.gte('created_at', start.toISOString());
            if (end) query = query.lte('created_at', end.toISOString());
            if (!isNaN(minAmt)) query = query.gte('amount', minAmt);
            if (!isNaN(maxAmt)) query = query.lte('amount', maxAmt);
    ...
    async loadPayments() {
        try {
            ...
            if (!tbody) return;
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 20px;">Loading pending payments...</td></tr>';
    ...
    addUser() {
        const modalHTML = `
            <div id="addUserModal" class="modal">
                <div class="modal-content edit-user-modal" style="max-height: 90vh; overflow-y: auto;">
                    <span class="close" onclick="this.closest('.modal').remove()">&times;</span>
                    <h2 style="color: #ffcc00; margin-bottom: 20px;">Add New User</h2>
                    <form class="user-form" id="addUserForm">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-input" id="newPassword" placeholder="Enter password" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Mobile</label>
                            <input type="text" class="form-input" id="newMobile" placeholder="Enter mobile number" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">WhatsApp (Optional)</label>
                            <input type="text" class="form-input" id="newEmail" placeholder="Enter WhatsApp number">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Initial Balance (${window.siteCurrency || '৳'})</label>
                            <input type="number" class="form-input" id="newBalance" value="0" step="0.01" min="0" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-input" id="newVipLevel">
                                <option value="Bronze">Bronze</option>
                                <option value="Silver">Silver</option>
                                <option value="Gold">Gold</option>
                                <option value="Platinum">Platinum</option>
                    ...
                    <div class="modal-content edit-user-modal" style="max-height: 90vh; overflow-y: auto;">
                        ...
                        <form class="user-form" id="editUserForm">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-input" id="editEmail" value="${user.email || ''}" placeholder="WhatsApp Number">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Mobile</label>
    ...
    async loadMembers(search = null) {
        try {
            ...
            const minBal = parseFloat(document.getElementById('memberMinBalance')?.value);
            const maxBal = parseFloat(document.getElementById('memberMaxBalance')?.value);
